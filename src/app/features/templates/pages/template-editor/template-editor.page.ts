 ...
 addField(type: FieldType): void {
  ...
  updateActionPlanPhase(
    ...
    this.definition.update(definition => ({
      ...definition,
      sections: definition.sections.map((section, index) => {
        if (index !== sectionIndex || !this.isActionPlanSection(section)) {
          return section;
        }
        return {
          ...section,
          phases: section.phases.map((phase, currentIndex) =>
            currentIndex === phaseIndex
              ? {
                  ...phase,
                  ...patch,
                  id: patch.label === undefined
                    ? phase.id
                    : this.createUniquePhaseId(patch.label, section.phases, phaseIndex)
                }
              : phase
          )
        };
  ...
  addFreePhase(sectionIndex: number): void {
    this.definition.update(definition => ({
      ...definition,
      sections: definition.sections.map((section, index) => {
        if (index !== sectionIndex || !this.isActionPlanSection(section) || section.methodology !== 'FREE') {
          return section;
        }
        const label = `Proyecto ${section.phases.length + 1}`;
        return {
          ...section,
          phases: [
            ...section.phases,
            {
              id: this.createUniquePhaseId(label, section.phases),
              label,
              description: ''
            }
          ]
  ...
  removeFreePhase(sectionIndex: number, phaseIndex: number): void {
    this.definition.update(definition => ({
      ...definition,
      sections: definition.sections.map((section, index) => {
        if (index !== sectionIndex || !this.isActionPlanSection(section) || section.methodology !== 'FREE' || section.phases.length <= 1) {
          return section;
        }
        return {
          ...section,
          phases: section.phases.filter((_, currentIndex) => currentIndex !== phaseIndex)
        };
      })
    }));
  }
...
section.fields.forEach(
  ...
  private createUniquePhaseId(
    label: string,
    phases: ChecklistActionPlanSection['phases'],
    ignoredIndex = -1
  ): string {
    const base = label
      .trim()
        ...
        phases: [
          {
            ...
            description:
              'Agrupa las acciones de este proyecto o frente de trabajo.'
          }
        ]
      };
  }
