...
export interface ActionPlanTemplateEcd {
  label?: string;
  date: string;
  completed?: boolean;
}
export interface ActionPlanTemplateRow {
  id: string;
  phaseId: string;
  parentId?: string;
  action: string;
  responsibleEmail: string;
  ecds: ActionPlanTemplateEcd[];
  progress: number;
  observations: string;
}
export interface ChecklistActionPlanSection {
  ...
  requireEcd?: boolean;
  /** Acciones migradas desde Excel/fotos que precargan una ejecución nueva. */
  initialActions?: ActionPlanTemplateRow[];
}
